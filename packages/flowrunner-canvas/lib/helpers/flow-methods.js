...
export const getNewNode = (node, flow, useNameFromNode) => {
    let indexFromName = -1;
...
export const getNewConnection = (nodeFrom, nodeTo, getNodeInstance, isEvent, thumbPositionRelativeToNode, positionToX, positionToY) => {
    const nodeFromPosition = FlowToCanvas.getStartPointForLine(nodeFrom, {
        x: nodeFrom.x,
        y: nodeFrom.y,
    }, undefined, undefined, isEvent || {}, getNodeInstance, thumbPositionRelativeToNode);
    let nodeToPosition = {
        x: 0,
        y: 0,
    };
    let nodeToName = undefined;
    if (nodeTo) {
        nodeToPosition = FlowToCanvas.getEndPointForLine(nodeTo, {
            x: nodeTo.x,
            y: nodeTo.y,
        }, nodeFrom, undefined, getNodeInstance);
...
//# sourceMappingURL=flow-methods.js.map