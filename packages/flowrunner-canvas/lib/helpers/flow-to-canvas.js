...
export class FlowToCanvas {
    static convertFlowPackageToCanvasFlow(flow, positionContext) {
        if (flow === undefined) {
            return [];
        }
        ...
        const resultFlow = flow.map((node, index) => {
            if (node.shapeType === 'line') {
                ...
                if (shartShapes.length >= 1 && endShapes.length >= 1) {
                    const startPosition = FlowToCanvas.getStartPointForLine(shartShapes[0], shartShapes[0], undefined, undefined, node, undefined, node.thumbPosition);
    ...
    static getStartPointForLine(startShape, newPosition, endNode, endNodePosition, lineNodeToCheckIfIsEvent, getNodeInstance, thumbPositionRelativeToNode, altThumbPositions) {
        ...
        if (shapeType == 'Html') {
            ...
            if (thumbPositionRelativeToNode == ThumbPositionRelativeToNode.bottom) {
                ...
                let element = document.querySelector('#' + startShape.name + ' .canvas__html-shape-thumb-startbottom');
                if (!bodyElement) {
                ...
                if (!!nodeAttachedToCenter) {
                    if (endNode && endNodePosition) {
                        ...
                        if (startShape.curveMode === 'arc') {
                            let resultPoints = [];
                            const controlPoints = calculateLineControlPoints(newPosition.x + (width || endNode.width || ShapeMeasures.htmlWidth) / 2, newPosition.y + (height || 0) / 2, endNodePosition.x + (widthEndNode || endNode.width || ShapeMeasures.htmlWidth) / 2, endNodePosition.y + (heightEndNode || endNode.height || 0) / 2, ThumbPositionRelativeToNode.default, ThumbPositionRelativeToNode.default, {
                                curveMode: 'arc',
                            });
                            const resultCount = cubicBezierAABB(newPosition.x + (width || startShape.width || ShapeMeasures.htmlWidth) / 2, newPosition.y + (height || 0) / 2, controlPoints.controlPointx1, controlPoints.controlPointy1, controlPoints.controlPointx2, controlPoints.controlPointy2, endNodePosition.x + (widthEndNode || endNode.width || ShapeMeasures.htmlWidth) / 2, endNodePosition.y + (heightEndNode || endNode.height || 0) / 2, newPosition.x - 0, newPosition.y - 0, newPosition.x + (width || startShape.width || ShapeMeasures.htmlWidth) + 0, newPosition.y + (height || 0) + 0, resultPoints);
    ...
    static getEndPointForLine(endNode, newPosition, startNode, startNodePosition, getNodeInstance, thumbPositionRelativeToNode, altThumbPositions) {
        const taskSettings = FlowToCanvas.getTaskSettings(endNode.taskType);
        const shapeType = FlowToCanvas.getShapeTypeUsingSettings(endNode.shapeType, endNode.taskType, endNode.isStartEnd, taskSettings);
        if (shapeType == 'Html') {
            let width = undefined;
            let height = 0;
            ...
            if (!!nodeAttachedToCenter) {
                if (startNode && startNodePosition) {
                    ...
                    if (startNode.curveMode === 'arc') {
                        let resultPoints = [];
                        const controlPoints = calculateLineControlPoints(startNodePosition.x + (widthStartNode || startNode.width || ShapeMeasures.htmlWidth) / 2, startNodePosition.y + (heightStartNode || startNode.height || 0) / 2, newPosition.x + (width || endNode.width || ShapeMeasures.htmlWidth) / 2, newPosition.y + (height || 0) / 2, ThumbPositionRelativeToNode.default, ThumbPositionRelativeToNode.default, {
                            curveMode: 'arc',
                        });
                        const resultCount = cubicBezierAABB(startNodePosition.x + (widthStartNode || startNode.width || ShapeMeasures.htmlWidth) / 2, startNodePosition.y + (heightStartNode || startNode.height || 0) / 2, controlPoints.controlPointx1, controlPoints.controlPointy1, controlPoints.controlPointx2, controlPoints.controlPointy2, newPosition.x + (width || endNode.width || ShapeMeasures.htmlWidth) / 2, newPosition.y + (height || 0) / 2, newPosition.x - 5, newPosition.y - 5, newPosition.x + (width || endNode.width || ShapeMeasures.htmlWidth) + 5, newPosition.y + (height || 0) + 5, resultPoints);
                        if (resultPoints.length >= 1) {
                            result = {
                                x: resultPoints[0],
                                y: resultPoints[1],
                        ...
                        const resultPointOnRect = pointOnRect(startNodePosition.x + (widthStartNode || startNode.width || ShapeMeasures.htmlWidth) / 2, startNodePosition.y + (heightStartNode || startNode.height || 0) / 2, newPosition.x - 5, newPosition.y - 5, newPosition.x + (width || endNode.width || ShapeMeasures.htmlWidth) + 5, newPosition.y + (height || 0) + 5, true);
    ...
    static getLinesForStartNodeFromCanvasFlow(flow, startNode, flowHashMap) {
    ...
    static getLinesForEndNodeFromCanvasFlow(flow, endNode, flowHashMap) {
    ...
    static getThumbEndPosition(shapeType, position, offset, positionRelativeToNode) {
    ...
    static getThumbStartPosition(shapeType, position, offset, positionRelativeToNode) {
        if (shapeType == 'Diamond') {
    ...
    static getHasInputs(shapeType, taskSettings) {
        ...
        if (taskSettings) {
            ...
            const allowedInputs = (_c = (_b = (_a = taskSettings.constraints) === null || _a === void 0 ? void 0 : _a.input) === null || _b === void 0 ? void 0 : _b.allowedInputs) !== null && _c !== void 0 ? _c : -1;
    ...
    static canHaveInputs(shapeType, taskSettings, flow, node, flowHashMap) {
        var _a, _b, _c;
    ...
    static getHasOutputs(shapeType, taskSettings) {
        var _a, _b, _c;
        if (taskSettings) {
            const allowedOutputs = (_c = (_b = (_a = taskSettings.constraints) === null || _a === void 0 ? void 0 : _a.output) === null || _b === void 0 ? void 0 : _b.allowedOutputs) !== null && _c !== void 0 ? _c : -1;
    ...
    static canHaveOutputs(shapeType, taskSettings, flow, node, flowHashMap) {
        var _a, _b, _c;
        if (flow === undefined || node === undefined || flowHashMap == undefined) {
            return false;
        }
    ...
    static canNodesConnect(inputNode, outputNode) {
        ...
        const allowedOutputTaskTypes = (_c = (_b = (_a = settings === null || settings === void 0 ? void 0 : settings.constraints) === null || _a === void 0 ? void 0 : _a.output) === null || _b === void 0 ? void 0 : _b.allowed) !== null && _c !== void 0 ? _c : [];
        const notAllowedOutputTaskTypes = (_f = (_e = (_d = settings === null || settings === void 0 ? void 0 : settings.constraints) === null || _d === void 0 ? void 0 : _d.output) === null || _e === void 0 ? void 0 : _e.notAllowed) !== null && _f !== void 0 ? _f : [];
        const settingsForNode = ShapeSettings.getShapeSettings(outputNode.taskType, outputNode);
        const allowedInputTaskTypesForOutputNode = (_j = (_h = (_g = settingsForNode === null || settingsForNode === void 0 ? void 0 : settingsForNode.constraints) === null || _g === void 0 ? void 0 : _g.input) === null || _h === void 0 ? void 0 : _h.allowed) !== null && _j !== void 0 ? _j : [];
        const notAllowedInputTaskTypesForOutputNode = (_m = (_l = (_k = settingsForNode === null || settingsForNode === void 0 ? void 0 : settingsForNode.constraints) === null || _k === void 0 ? void 0 : _k.input) === null || _l === void 0 ? void 0 : _l.notAllowed) !== null && _m !== void 0 ? _m : [];
        ...
        if (allowedInputTaskTypesForOutputNode.length > 0 &&
...
FlowToCanvas.createFlowHashMap = flow => {
    const startPerf = performance.now();
...
//# sourceMappingURL=flow-to-canvas.js.map