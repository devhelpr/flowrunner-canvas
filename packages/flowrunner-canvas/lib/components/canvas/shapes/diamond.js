...
import { Group, Text, Shape as KonvaShape, RegularPolygon, Image as KonvaImage, Arrow as KonvaArrow, Circle as KonvaCircle, } from 'react-konva';
...
export const Diamond = React.forwardRef((props, ref) => {
    const [image] = useImage("/svg/cog.svg");
    const groupRef = useRef(null);
    const regularPolygonRef = useRef(null);
    ...
    return _jsx(_Fragment, { children: _jsxs(Group, { x: props.x, y: props.y, ref: groupRef, onDragMove: props.onDragMove, onDragEnd: props.onDragEnd, draggable: false, transformsEnabled: "position", onClick: props.onClickShape, onMouseOver: props.onMouseOver, onMouseOut: props.onMouseOut, onTouchStart: props.onTouchStart, onTouchMove: props.onTouchMove, onTouchEnd: props.onTouchEnd, onMouseDown: props.onMouseStart, onMouseMove: props.onMouseMove, onMouseUp: props.onMouseEnd, onMouseLeave: props.onMouseLeave, opacity: props.canvasHasSelectedNode && !props.isSelected && !props.isConnectedToSelectedNode ? 1 : 1, children: [_jsx(_Fragment, { children: settings.subShapeType === "Loop" ? _jsxs(_Fragment, { children: [_jsx(KonvaCircle, { x: ShapeMeasures.diamondSize / 2, y: ShapeMeasures.diamondSize / 2, radius: ShapeMeasures.diamondSize, strokeWidth: 0, opacity: 1, width: ShapeMeasures.diamondSize, height: ShapeMeasures.diamondSize, fill: props.isSelected ? settings.fillSelectedColor : settings.fillColor, transformsEnabled: "position", perfectDrawEnabled: false }), _jsx(KonvaShape, { x: ShapeMeasures.diamondSize / 2, y: ShapeMeasures.diamondSize / 2, width: ShapeMeasures.diamondSize, height: ShapeMeasures.diamondSize, stroke: strokeColor, strokeWidth: 4, ref: shapeRef, transformsEnabled: "position", sceneFunc: (context, shape) => {
                                    context.beginPath();
                                    context.lineWidth = 3;
                                    context.arc(0, 0, ShapeMeasures.diamondSize / 2, 1.5 * Math.PI, 0.95 * Math.PI, false);
                                    context.stroke();
                                } }), _jsx(KonvaArrow, { x: 0.25, y: 12 + (ShapeMeasures.diamondSize / 2), points: [
                                    0.5,
                                    0,
                                    0,
                                    -4
                        ...
                        _jsx(RegularPolygon, { x: ShapeMeasures.diamondSize / 2, y: ShapeMeasures.diamondSize / 2, stroke: strokeColor, strokeWidth: 4, cornerRadius: 4, sides: 4, transformsEnabled: "position", ref: regularPolygonRef, radius: ShapeMeasures.diamondSize, width: ShapeMeasures.diamondSize, height: ShapeMeasures.diamondSize, fill: fillColor, perfectDrawEnabled: false }) }), _jsx(Text, { x: 10, y: 0, text: labelText, align: 'center', width: ShapeMeasures.diamondSize - 20, height: ShapeMeasures.diamondSize, verticalAlign: "middle", listening: false, wrap: "word", transformsEnabled: "position", textDecoration: textDecoration, fontSize: 18, ellipsis: true, fill: settings.textColor, perfectDrawEnabled: false }), !!settings.hasConfigMenu && _jsx(KonvaImage, { image: image, pathColor: settings.textColor, width: Math.round(ShapeMeasures.diamondSize / 8), height: Math.round(ShapeMeasures.diamondSize / 8), keepRatio: true, x: Math.round((ShapeMeasures.diamondSize / 2) - ShapeMeasures.diamondSize / 16), y: 16, perfectDrawEnabled: false, transformsEnabled: "position", onClick: props.onClickSetup })] }) });
});
