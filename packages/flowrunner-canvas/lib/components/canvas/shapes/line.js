...
import { ModifyShapeEnum, ShapeStateEnum, ThumbFollowFlow, ThumbPositionRelativeToNode } from './shape-types';
...
export const Line = React.forwardRef((props, ref) => {
    const [fillColor, setFillColor] = useState(props.isConnectionWithVariable ? "#e08000" :
        ...
        modifyShape: (action, parameters) => {
            switch (+action) {
                ...
                case ModifyShapeEnum.SetPoints: {
                    ...
                    if (textRef2.current) {
                        const center = getBezierXY(0.5, parameters.points[0], parameters.points[1], parameters.points[2], parameters.points[3], parameters.points[4], parameters.points[5], parameters.points[6], parameters.points[7]);
    ...
    return _jsxs(_Fragment, { children: [_jsxs(Group, { listening: !props.noMouseEvents, transformsEnabled: "position", children: [_jsx(KonvaLine, { ref: bgLineRef, points: [props.xstart, props.ystart,
                        ...
                        }, fill: "#FF0000" })] }), props.hasStartThumb !== undefined && !!props.hasStartThumb && props.shapeRefs && _jsx(ThumbsStart, { position: FlowToCanvas.getThumbStartPosition("", { x: props.xstart, y: props.ystart }, 0), name: "", taskType: "", shapeType: "", node: props.lineNode, ref: ref => {
                    if (props.shapeRefs) {
                        props.shapeRefs["thumbstart_line_" + props.lineNode.name] = ref;
                    }
                }, isSelected: false, isConnectedToSelectedNode: false, canvasHasSelectedNode: false, thumbPositionRelativeToNode: ThumbPositionRelativeToNode.default, onMouseConnectionStartOver: (event) => props.onMouseConnectionStartOver(props.lineNode, false, event), onMouseConnectionStartOut: (event) => props.onMouseConnectionStartOut(props.lineNode, false, event), onMouseConnectionStartStart: (event) => props.onMouseConnectionStartStart(props.lineNode, false, "", ThumbFollowFlow.default, ThumbPositionRelativeToNode.default, event), onMouseConnectionStartMove: (event) => props.onMouseConnectionStartMove(props.lineNode, false, event), onMouseConnectionStartEnd: (event) => props.onMouseConnectionStartEnd(props.lineNode, false, ThumbPositionRelativeToNode.default, event), getNodeInstance: props.getNodeInstance }), props.hasEndThumb !== undefined && !!props.hasEndThumb && props.shapeRefs && _jsx(Thumbs, { position: FlowToCanvas.getThumbEndPosition("", { x: props.xend, y: props.yend }), name: "", taskType: "", shapeType: "", node: undefined, ref: ref => {
                    if (props.shapeRefs) {
