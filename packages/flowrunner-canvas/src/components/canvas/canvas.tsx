...
export const Canvas = (props: CanvasProps) => {
  ...
  let connectionXEnd = useRef(0);
  let connectionYEnd = useRef(0);
            ...
            if (lineRef && lineRef.modifyShape(ModifyShapeEnum.GetShapeType, {}) === 'line') {
              if (touchedNodesLocal.current[touchNodeId]) {
                if (
                  ...
                  selectedNodeRef.current.node.name === touchNodeId
                ) {
                  lineRef.modifyShape(ModifyShapeEnum.SetState, {
                    state: ShapeStateEnum.SelectedTouched,
                  });
                } else {
                  lineRef.modifyShape(ModifyShapeEnum.SetState, {
  ...
  const setHtmlElementsPositionAndScale = (
    stageX,
    stageY,
    stageScale,
    newX?: number,
  ...
  const setHtmlGlobalScale = (stageX, stageY, stageScale) => {
    if (htmlWrapper && htmlWrapper.current) {
      (htmlWrapper.current as any).style.transform =
        'translate(' + stageX + 'px,' + stageY + 'px) ' + 'scale(' + stageScale + ',' + stageScale + ') ';
    }
  };
  ...
  useEffect(() => {
    ...
    (props.useSelectedNodeStore as any).subscribe(
      ...
      (node: any, previousNode: any) => {
        console.log('useSelectedNodeStore.subscribe node previousNode', node, previousNode);
        selectedNodeRef.current = node;

        if (previousNode && previousNode.node) {
          const nodeElement = document.querySelector(`#${previousNode.node.name}`);
          if (nodeElement) {
            nodeElement.classList.remove('canvas__html-shape--selected');
          }
          const barElement = document.querySelector(`#${previousNode.node.name} .canvas__html-shape-bar`);
          if (barElement) {
            barElement.classList.remove('canvas__html-shape-bar--selected');
          }
          const htmlElement = document.querySelector(`#${previousNode.node.name}.canvas__html-shape`);
          if (htmlElement) {
            htmlElement.classList.add('canvas__html-shape');
          } else {
            const shapeRef = shapeRefs.current[previousNode.node.name];
            if (shapeRef) {
              if (touchedNodesLocal.current[previousNode.node.name] === true) {
        ...
        if (node && node.node) {
          console.log('modifyshape node', node.node.name);
          const nodeElement = document.querySelector(`#${node.node.name}`);
          if (nodeElement) {
            nodeElement.classList.add('canvas__html-shape--selected');
          }
          const barElement = document.querySelector(`#${node.node.name} .canvas__html-shape-bar`);
          if (barElement) {
            barElement.classList.add('canvas__html-shape-bar--selected');
          }
          const htmlElement = document.querySelector(`#${node.node.name}.canvas__html-shape`);
          if (htmlElement) {
            htmlElement.classList.add('canvas__html-shape');
          } else {
            const shapeRef = shapeRefs.current[node.node.name];
            if (shapeRef) {
              if (touchedNodesLocal.current[node.node.name] === true) {
                shapeRef.modifyShape(ModifyShapeEnum.SetState, {
                  state: ShapeStateEnum.SelectedTouched,
                });
              } else {
  ...
  const getOutputConnectNodes = (nodeName: any) => {
    let outputConnections: any[] = [];
    if (!flowStoreHashMap.current || !flowStoreFlow.current) {
      return [];
    }
    const mappedNode = (flowStoreHashMap.current as any).get(nodeName);
    if (mappedNode) {
      const outputs = mappedNode.start;
      if (outputs && outputs.length > 0) {
        outputs.forEach((outputIndex) => {
          if (flowStoreFlow.current) {
            const outputLineNode = flowStoreFlow.current[outputIndex];
            outputConnections.push(outputLineNode);
            if (outputLineNode.endshapeid) {
              outputConnections = [...outputConnections, ...getOutputConnectNodes(outputLineNode.endshapeid)];
            }
          }
        });
      }
  ...
  const getDirectOutputConnectNodes = (nodeName: any) => {
    let outputConnections: any[] = [];
    if (!flowStoreHashMap.current || !flowStoreFlow.current) {
      return [];
    }
  ...
  const nodeStateTimeoutCallback = () => {
    ...
    nodeStateList.current.forEach((nodeState) => {
      ...
      let connectionStream: any[] | undefined = undefined;
      ...
      if (mappedNode && flowStoreFlow.current) {
        ...
        if (node && node.resetOutputPathOnError && nodeState.nodeState === 'error') {
          ...
          resetNodes.push(nodeState.name);
        }

        const outputConnections = getDirectOutputConnectNodes(nodeState.nodeName);
        if (outputConnections) {
          outputConnections.forEach((node) => {
            if (wasIncreased.indexOf(node.name) < 0) {
              const lineRef = shapeRefs.current[node.name];

              if (lineRef && lineRef.modifyShape(ModifyShapeEnum.GetShapeType, {}) === 'line') {
                lineRef.modifyShape(ModifyShapeEnum.IncreaseDashOffset);
              }

              wasIncreased.push(node.name);
            }
          });
        }
    ...
    Object.keys(touchedNodesLocal.current).forEach((touchNodeId: string) => {
      if (resetNodes[touchNodeId]) {
        nodesStateLocal.current[touchNodeId] = '';
        touchedNodesLocal.current[touchNodeId] = false;
      }
  ...
  const nodeStateObserver = (nodeName: string, nodeState: string, _touchedNodes: any, payload: any) => {
    ...
    if (nodeState === 'RESET_ALL') {
      ...
      Object.keys(shapeRefs.current).forEach((nodeName: string) => {
    ...
    (nodeStateTimeout.current as any) = setTimeout(nodeStateTimeoutCallback, 5);
  ...
  useLayoutEffect(() => {
    ...
    if (props.flowHasNodes) {
      ...
      flowStore.flow.map((node, index) => {
        if (
          node.taskType === 'TimerTask' ||
          (node.taskType == 'DebugTask' && node.visualizer == 'animatedgridcanvas')
        ) {
  ...
  }, [props.flowState, flowStore.flow, flowStore.flowHashmap]);
...
*/
        ...
        if (
          determineStartPosition(
            event.evt ? event.currentTarget : positionContext.getPosition(node.id),
    ...
    if (mappedNode) {
      ...
      if (outputs && outputs.length > 0) {
        outputs.forEach((outputIndex) => {
          ...
          if (outputLineNode.endshapeid && draggingMultipleNodes.current.indexOf(outputLineNode.endshapeid) < 0) {
    ...
    if (mappedNode) {
      ...
      if (inputs && inputs.length > 0) {
        inputs.forEach((outputIndex) => {
          const outputLineNode = flowStore.flow[outputIndex];
          if (outputLineNode.startshapeid && draggingMultipleNodes.current.indexOf(outputLineNode.startshapeid) < 0) {
...
*/
    ...
    if (touching.current && interactionState.current === InteractionState.idle) {
      ...
      if (node.shapeType === 'Section' && node.taskType === 'Annotation') {
  ...
  const connectConnectionToNode = (node, thumbPositionRelativeToNode?) => {
    ...
    if (connectionNodeEventName.current !== '' && !isNaN(connectionNodeEvent.current as number)) {
  ...
  const removeOrAddNodeToSections = (node: any) => {
    if (stage && stage.current) {
      ...
      if (stageInstance) {
        ...
        let shapeType = FlowToCanvas.getShapeTypeUsingSettings(
          node.shapeType,
          node.taskType,
          node.isStartEnd,
          taskSettings,
      ...
      if (stage && stage.current) {
        ...
        if (
          ...
          if (draggingMultipleNodes.current && draggingMultipleNodes.current.length > 0) {
              ...
              if (lineNode.startshapeid && flowStore.flowHashmap) {
                ...
                if (startNode) {
                  setNewPositionForNode(
                    event,
                    startNode,
                    shapeRefs.current[startNode.name],
                    event.evt && event.evt.screenX
                      ? {
                          x: event.evt.screenX,
                          y: event.evt.screenY,
            ...
            setNewPositionForNode(
              event,
              touchNode.current as any,
              shapeRefs.current[(touchNode.current as any).name],
              undefined,
              true,
              false,
              true,
  ...
  const moveSelectRectangle = (event) => {
    ...
    if (stage && stage.current && (selectingRectInfo.current || selectingViaAnnotationRef.current)) {
      ...
      if (stageInstance) {
        ...
        if (selectingViaAnnotationRef.current) {
          console.log(
            'moveSelectRectangle : selectingViaAnnotationRef.current',
            selectingViaAnnotationRef.current,
          ...
          (selectingRectRef.current as any).x(
            (selectingRectInfo.current.x + offsetX - stageInstance.x()) / scaleFactor,
          );
          (selectingRectRef.current as any).y(
            (selectingRectInfo.current.y + offsetY - stageInstance.y()) / scaleFactor,
          );
        }
      }
    }
  ...
  const updateSelectRectangle = (event) => {
    if (stage && stage.current && (selectingRectInfo.current || selectingViaAnnotationRef.current)) {
      ...
      if (stageInstance) {
        ...
        if (selectingViaAnnotationRef.current) {
          console.log(
            'updateSelectRectangle : selectingViaAnnotationRef.current',
            selectingViaAnnotationRef.current,
            offsetPosition,
            '->',
  ...
  const handleMultiSelectCursor = (event) => {
    ...
    if (stage && stage.current && selectingRectInfo.current) {
      ...
      if (stageInstance) {
        const touchPos = getCurrentPosition(event);
        if (
          selectingRectInfo.current.x <= touchPos.x &&
          ...
          if (connectionNodeThumbs.current === '') {
            ...
            if (lineRef) {
              let controlPoints = calculateLineControlPoints(
                connectionXStart.current,
                connectionYStart.current,
                newPosition.x,
                newPosition.y,
  ...
  const onTouchMove = (node, event) => {
    ...
    if (node.shapeType !== 'Line') {
      ...
      setNewPositionForNode(
        event,
        node,
        shapeRefs.current[node.name],
        event.evt.touches.length > 0
          ? {
    ...
    if (event.currentTarget) {
      if (mouseDragging.current) {
        setNewPositionForNode(
          ...
          node,
          shapeRefs.current[node.name],
          event.evt.changedTouches.length > 0
            ? {
  ...
  const onMouseConnectionStartStart = (
    ...
    // TODO : check if node has outputs... and yes, then use that for reconnection
    
    if (
      selectedNodeRef.current !== undefined &&
      (selectedNodeRef.current as any).shapeType === 'Line' &&
      (selectedNodeRef.current as any).name !== node.name
    ) {
      console.log('onMouseConnectionStartStart', node.name, (selectedNodeRef.current as any).name);
      return;
    }*/
    ...
    if (node && node.shapeType === 'Line') {
      ...
      let lineNode: any | undefined = node;
      if (
        selectedNodeRef.current &&
        selectedNodeRef.current.node &&
        selectedNodeRef.current.node.shapeType === 'Line'
      ) {
      ...
      if (mappedNode) {
        if (mappedNode.start && (mappedNode.start?.length ?? -1) > 0) {
          ...
          if (
            ...
            mappedNode.start.forEach((connectionNodeIndex) => {
              const _connectionNode = flowStore.flow[connectionNodeIndex];
              if (_connectionNode.name === selectedNodeRef.current.node.name) {
                lineNode = _connectionNode;
              }
            });
          }
    ...
    if (stage && stage.current) {
      ...
      if (stage) {
        ...
        //connectionXEnd.current = newPosition.x;
        //connectionYEnd.current = newPosition.x;
      }
    }
  ...
  const onMouseConnectionStartEnd = (
    node,
    nodeEvent,
    thumbPositionRelativeToNode: ThumbPositionRelativeToNode,
    event,
    ...
    console.log('onMouseConnectionStartEnd', connectionNodeThumbsLineNode.current);
    if (isConnectingNodesByDraggingLocal.current && touchNode.current && node) {
      if (connectionNodeThumbsLineNode.current) {
        if (connectionNodeThumbsLineNode.current.shapeType === 'Line') {
          return;
        }
      ...
      if (!FlowToCanvas.canNodesConnect(touchNode.current, node)) {
        console.log('onMouseConnectionStartEnd not allowed');
        return false;
      }
  ...
  const onMouseConnectionEndOver = (
    node,
    nodeEvent,
    event,
    thumbPositionRelativeToNode?: ThumbPositionRelativeToNode,
    ...
    const settings = ShapeSettings.getShapeSettings(node.taskType, node);
    if (node && node.shapeType !== 'Line' && touching.current && touchNode.current) {
      const allowedInputs = FlowToCanvas.getAllowedInputs(node.shapeType, settings);
      if (
        allowedInputs === 0 ||
        !FlowToCanvas.canHaveInputs(node.shapeType, settings, flowStore.flow, node, flowStore.flowHashmap)
  ...
  const onClickShape = (node, event) => {
    ...
    if (
      (!!canvasMode.isConnectingNodes || !!shiftDown.current) &&
      selectedNodeRef.current !== undefined &&
      (selectedNodeRef.current as any).shapeType !== 'Line'
    ) {
      ...
      if (stageInstance !== undefined) {
        ...
        if (
          ...
          if (stageContainerElement !== null && bodyElement !== null) {
            ...
            newPos.y = -yMin * scale + (stageHeight + (props.isEditingInModal ? 0 : 64)) / 2 - (flowHeight * scale) / 2;
  ...
  const clickStage = (event) => {
    ...
    if (!nodeIsSelected && canvasMode.selectedTask !== undefined && canvasMode.selectedTask !== '') {
      if (!canvasMode.isConnectingNodes) {
        if (stage && stage.current) {
          let stageInstance = (stage.current as any).getStage();
          const position = (stageInstance as any).getPointerPosition();
          ...
          let newNode = getNewNode(
            {
              ...
              taskType: taskType,
              shapeType:
                canvasMode.selectedTask == 'IfConditionTask'
                  ? 'Diamond'
                  : shapeSetting.shapeType
                  ? shapeSetting.shapeType
                  : 'Rect',
              x: (position.x - stageInstance.x()) / scaleFactor,
              y: (position.y - stageInstance.y()) / scaleFactor,
      ...
      if (stage && stage.current) {
        ...
        repoFlow = repoFlow.map((node: any) => {
          if (node.shapeType !== 'Line') {
            ...
            return { ...node, name: renameIdMap[node.name], id: renameIdMap[node.name] };
          }
          return node;
        });
        repoFlow = repoFlow.map((node: any) => {
  ...
  const addTaskClassNameToCanvas = (
    event: any,
    taskClassName: string,
    extraNodeProperties?: any,
    draggableElementRect?: any,
  ) => {
    if (stage && stage.current) {
      const stageInstance = (stage.current as any).getStage();
        ...
        if (!canvasMode.isConnectingNodes) {
          if (stage && stage.current) {
            ...
            if (shapeType === 'Html' && draggableElementRect) {
              if (props.getNodeInstance) {
                const left =
                  Math.round((draggableElementRect.left + draggableElementRect.right) / 2) -
                  canvasTopLeftPositionRef.current.x;
                const top =
                  Math.round((draggableElementRect.top + draggableElementRect.bottom) / 2) -
                  canvasTopLeftPositionRef.current.y;
  ...
  const movingExistingOrNewNodeOnCanvas = (
    dropX: number,
    dropY: number,
    taskTypeBeingDraggedOnStage?: string,
    isConnectingToExistingNode?: boolean,
    existingNode?: any,
    noScale?: boolean,
  ) => {
      ...
      if (stageInstance) {
        ...
        flowStore.flow.forEach((node) => {
          if (node.shapeType === 'Line') {
            ...
            const rightPosition = FlowToCanvas.getStartPointForLine(
              node,
              {
                x: nodePosition.x,
                y: nodePosition.y,
            ...
            const bottomPosition = FlowToCanvas.getStartPointForLine(
              node,
              {
                x: nodePosition.x,
                y: nodePosition.y,
            ...
            const leftPosition = FlowToCanvas.getEndPointForLine(
              node,
              {
                x: nodePosition.x,
                y: nodePosition.y,
            ...
            const topPosition = FlowToCanvas.getEndPointForLine(
              node,
              {
                x: nodePosition.x,
                y: nodePosition.y,
              ...
              undefined,
              props.getNodeInstance,
              ThumbPositionRelativeToNode.top,
            );

            let distanceTop = -1;
            let distanceBottom = -1;
            if (node.shapeType === 'Diamond') {
              const topPositionDiamond = FlowToCanvas.getStartPointForLine(
                node,
                {
                  x: nodePosition.x,
                  y: nodePosition.y,
              ...
              const bottomPositionDiamond = FlowToCanvas.getStartPointForLine(
                node,
                {
                  x: nodePosition.x,
                  y: nodePosition.y,
        ...
        if (isNodeConnection) {
          ...
          if (!isNodeConnection && isInputToNode) {
            ...
            const canHaveInputs = FlowToCanvas.canHaveInputs(
              closestNode.shapeType,
              settings,
              flowStore.flow,
              closestNode,
              flowStore.flowHashmap,
            );
          ...
          if (!isNodeConnection && isOutputToNode) {
            ...
            const canHaveOutputs = FlowToCanvas.canHaveOutputs(
              closestNode.shapeType,
              settings,
              flowStore.flow,
              closestNode,
          ...
          if (!isNodeConnection && lineRef) {
            ...
            if (closestNode.shapeType === 'Diamond' && nodeOrientation !== ThumbPositionRelativeToNode.default) {
              lineStartPosition = FlowToCanvas.getStartPointForLine(
                closestNode,
                {
                  x: nodePosition.x,
                  y: nodePosition.y,
              ...
              lineStartPosition = FlowToCanvas.getEndPointForLine(
                closestNode,
                {
                  x: nodePosition.x,
                  y: nodePosition.y,
  ...
  return (
    <>
      ...
      >
        ...
        ></Taskbar>
        <DragOverlay className="taskbar__task-dragging" zIndex={20000} dropAnimation={null}>
          {activeId ? <DragginTask id={activeId} style={{}} children={<></>} listeners={{}} /> : null}
        </DragOverlay>
        <div
        ...
        >
          <div ref={setNodeRef} style={droppableStyle}>
            <PositionContext.Consumer>
              {(value) => (
                ...
                >
                  <PositionProvider positionContext={value} isBridged={true}>
                                  ...
                                  if (endIndex >= 0) {
                                    ...
                                    if (endNode) {
                                      ...
                                      let newEndPosition = FlowToCanvas.getEndPointForLine(
                                        ...
                                        undefined,
                                        props.getNodeInstance,
                                        (node.thumbEndPosition as ThumbPositionRelativeToNode) ||
                                          ThumbPositionRelativeToNode.default,
                                      );
          ...
          <div
            ref={(ref) => ((htmlWrapper as any).current = ref)}
      ...
      {showNodeSettings && (
        <EditNodeSettings
          ...
          flowrunnerConnector={props.flowrunnerConnector}
          onClose={onCloseEditNodeSettings}
        ></EditNodeSettings>
      )}
      {showNodeEdit && (
        <EditNodePopup
