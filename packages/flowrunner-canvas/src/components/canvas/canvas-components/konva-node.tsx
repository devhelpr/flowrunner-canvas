  ...
  if (props.node.shapeType !== 'Line' && Shape && position) {
    ...
    return (
      <React.Fragment>
        ...
        {(shapeType === 'Rect' || shapeType === 'Html') && FlowToCanvas.getHasOutputs(shapeType, settings) && (
          <ThumbsStart
        ...
        {shapeType === 'Html' && FlowToCanvas.getHasOutputs(shapeType, settings) && (
          <ThumbsStart
            position={FlowToCanvas.getThumbStartPosition(shapeType, position, 0)}
            name={props.node.name}
            taskType={props.node.taskType}
        ...
        {shapeType === 'Html' && FlowToCanvas.getHasInputs(shapeType, settings) && (
          <Thumbs
            position={FlowToCanvas.getThumbEndPosition(shapeType, position, 0, ThumbPositionRelativeToNode.top)}
            name={props.node.name}
            taskType={props.node.taskType}
        ...
        {shapeType === 'Diamond' && FlowToCanvas.getHasOutputs(shapeType, settings) && (
          <ThumbsStart
            position={FlowToCanvas.getThumbStartPosition(
              shapeType,
              position,
              0,
            ...
            onMouseConnectionStartMove={(event) => props.onMouseConnectionStartMove(props.node, false, event)}
            onMouseConnectionStartEnd={(event) =>
              props.onMouseConnectionStartEnd(
                props.node,
                false,
                !settings.altThumbPositions ? ThumbPositionRelativeToNode.top : ThumbPositionRelativeToNode.left,
                event,
          ...
          settings.events.map((event, eventIndex) => {
            return (
              <ThumbsStart
                ...
                onMouseConnectionStartEnd={(event) =>
                  props.onMouseConnectionStartEnd(props.node, eventIndex, ThumbPositionRelativeToNode.default, event)
                }
                getNodeInstance={props.getNodeInstance}
              ></ThumbsStart>
            );
