...
export const Line = React.forwardRef((props: LineTypeProps, ref: any) => {
  const [fillColor, setFillColor] = useState(
    props.isConnectionWithVariable ? '#e08000' : props.isConnectionWithFunction ? '#0080e0' : '#000000',
  );
  ...
  const getSelectedColor = (colorString, defaultColor) => {
    switch (colorString) {
      case 'yellow': {
        return '#f0e938';
      }
  ...
  const getColor = (colorString, defaultColor) => {
    switch (colorString) {
      case 'yellow': {
        return '#fbf791';
      }
  ...
  useEffect(() => {
    ...
    _fillColor = props.isSelected
      ? getSelectedColor(props.lineNode?.color, '#606060')
      : getColor(props.lineNode?.color, '#000000');

    // if (props.lineNode?.color === 'red') {
    //   _fillColor = props.isSelected ? '#800000' : '#e00000';
    // } else if (props.lineNode?.color === 'green') {
    ...
    if (!props.isSelected && props.canvasHasSelectedNode) {
      if (props.selectedNodeName != props.startNodeName && props.selectedNodeName != props.endNodeName) {
        //_opacity = 0.15;
      }
    }
  ...
  return (
    <>
      <Group listening={!props.noMouseEvents} transformsEnabled={'position'}>
        <KonvaLine
          ref={bgLineRef}
          points={[
            props.xstart,
      ...
      {props.hasStartThumb !== undefined && !!props.hasStartThumb && props.shapeRefs && (
        <ThumbsStart
          ...
          onMouseConnectionStartStart={(event) =>
          ...
          onMouseConnectionStartEnd={(event) =>
            props.onMouseConnectionStartEnd(props.lineNode, false, ThumbPositionRelativeToNode.default, event)
          }
          getNodeInstance={props.getNodeInstance}
        ></ThumbsStart>
      )}
      {props.hasEndThumb !== undefined && !!props.hasEndThumb && props.shapeRefs && (
        <Thumbs
