...
export const useSetPositionHook = (
  ...
  const setNewPositionForNode = (
    event,
    node,
    group,
    position?: any,
    isCommitingToStore?: boolean,
    linesOnly?: boolean,
    doDraw?: boolean,
    skipSetHtml?: boolean,
    isEndNode?: boolean,
    ...
    if (newPosition && !linesOnly) {
      ...
      if (shapeRefs.current[node.name]) {
        ...
        if (currentGroupThumbs) {
          let thumbPosition;
          if (diamondThumb === 2) {
            thumbPosition = FlowToCanvas.getThumbEndPosition(
              shapeType,
              newPosition,
              0,
        ...
        if (currentGroupThumbs) {
          const thumbPosition = FlowToCanvas.getThumbEndPosition(
            shapeType,
            newPosition,
            0,
            ThumbPositionRelativeToNode.top,
        ...
        if (currentGroupThumbs) {
          let thumbPosition;
          if (diamondThumb === 2) {
            thumbPosition = FlowToCanvas.getThumbStartPosition(
              shapeType,
              newPosition,
              0,
              ThumbPositionRelativeToNode.left,
            );
          } else {
            thumbPosition = FlowToCanvas.getThumbStartPosition(
              shapeType,
        ...
        if (currentGroupThumbs) {
          ...
          if (diamondThumb === 2) {
            thumbPosition = FlowToCanvas.getThumbStartPosition(
              ...
              newPosition,
              0,
              ThumbPositionRelativeToNode.right,
            );
          } else {
            thumbPosition = FlowToCanvas.getThumbStartPosition(
              shapeType,
    ...
    if (startLines) {
      startLines.map(lineNode => {
        ...
        if (endNode) {
          ...
          const newEndPosition = FlowToCanvas.getEndPointForLine(
            endNode,
            {
              x: positionNode.x,
              y: positionNode.y,
