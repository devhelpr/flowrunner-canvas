...
export const Flow = (props: IFlowProps) => {
  ...
  useEffect(() => {
    ...
    if (internalFlow.length != props.flow.length) {
      console.log(
        'internalFlow.length != props.flow.length : setInternalFlow',
        internalFlow.length,
        props.flow.length,
      ...
      if (changed || !!props.flowrunnerConnector.forcePushToFlowRunner) {
        console.log(
          ...
          changedNode,
          changedNodeProperty,
          props.flowrunnerConnector.forcePushToFlowRunner,
          props.flow,
        );
  ...
  useEffect(() => {
    ...
    console.log('flow pushFlowToFlowrunner', internalFlow, performance.now() - perfstart + 'ms');
  }, [internalFlow]);
  return <></>;
};
