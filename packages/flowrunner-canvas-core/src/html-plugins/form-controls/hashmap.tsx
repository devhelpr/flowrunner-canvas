...
export const HashMap = (props: IFormControlProps) => {
  ...
  return (
    <div className="form-group">
      <label>
        <strong>{metaInfo.label || metaInfo.fieldName || node.name}</strong>
      </label>
      {Object.keys(formControl.value).map((objectPropertyName, index) => {
        return (
          <div className="form-control__hash-map" key={'hashmap-' + objectPropertyName + index}>
            <div
              className="form-control__hash-map-heading row no-gutters position-relative"
              key={'input' + metaInfo.fieldName + index}
            >
              ...
              <a
                ...
                onFocus={onFocus}
                onClick={(event) => {
                  (event) => deleteItem(objectPropertyName, event);
                }}
              ...
              <a
                href="#"
                onClick={(event) => {
                  editItem(objectPropertyName, event);
                }}
            ...
            {editProperty == objectPropertyName && (
              <div className="form-control__hash-map-details">
                <FormNodeHtmlPlugin
                  node={{
      ...
      {isAdding ? (
        <div className="form-control__hash-map">
          ...
          <div className="form-control__object-list-node-controls">
            <button onFocus={onFocus} onClick={onAppendValue} className="btn btn-primary mr-2">
              Add
            </button>
            <button onFocus={onFocus} onClick={onCloseAppendValue} className="btn btn-outline-primary">
              Close
            </button>
          </div>
        </div>
