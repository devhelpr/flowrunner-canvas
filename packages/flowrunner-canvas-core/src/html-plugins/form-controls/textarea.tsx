...
export const Textarea = (props: IFormControlProps) => {
  ...
  let formControl = useFormControl(
    metaInfo.storeAsBase64 ? base64Decode(props.value) : props.value,
    metaInfo,
    props.onChange,
  );
  ...
  useLayoutEffect(() => {
    formControl.setValue(metaInfo.storeAsBase64 ? base64Decode(props.value) : props.value);
  ...
  const onChange = (event) => {
    ...
    formControl.onChange({
      target: {
        value: metaInfo.storeAsBase64 ? btoa(event.target.value) : event.target.value,
      },
    });
