...
export const PresetManager = (props: IPresetManagerProps) => {
  ...
  const loadInitialPresets = useCallback(async () => {
    try {
      ...
      console.log('Error in loadInitialPresets', err);
      setPresets([]);
    }
  }, [get, response, flow]);
  ...
  const onSelectPreset = useCallback(
    (event) => {
      ...
      if (event.target.value != '') {
        try {
          get(`/api/get-preset?flowId=${flow.flowId}&nodeName=${props.node.name}&preset=${event.target.value}`).then(
            (preset) => {
          ...
          console.log('Error in onSelectPreset', err);
          props.onSetData({});
        }
      }
      return false;
  ...
  const onSavePreset = useCallback(
    (event) => {
      ...
      if (props.onGetData && selectedPreset != '') {
        ...
        post(
          `/api/save-preset?flowId=${flow.flowId}&name=${presetName}&nodeName=${props.node.name}&preset=${selectedPreset}`,
          { data: data },
        ).then(() => {
          //
        });
  ...
  const onPresetName = (name: string) => {
    ...
    post(`/api/save-preset?flowId=${flow.flowId}&nodeName=${props.node.name}&name=${name}&preset=${newId}`, {
      data: data,
    }).then(() => {
      //
    });
    ...
    <>
      <form className="form w-100">
        <div className="form-group">
          ...
          {!error && (
            <select className="form-control form-select" onChange={onSelectPreset} value={selectedPreset}>
              ...
              {presets &&
                presets.map((preset: any, index) => {
                  ...
                  return (
                    <option key={'preset-' + index} value={preset.preset}>
                      {preset.name}
                    </option>
                  );
                })}
        ...
        <div className="form-group">
          <button onClick={onSavePreset} className="btn btn-primary mr-2">
            Save
          </button>
          <button onClick={onNewPreset} className="btn btn-outline-primary">
            New preset
          </button>
        </div>
      </form>
