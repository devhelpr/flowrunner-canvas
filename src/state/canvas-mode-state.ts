...
let storeHandler = (set : SetState<ICanvasModeState>) : ICanvasModeState => {
	return {
		...
		flowsPlayground: [],
		flowsWasm: [],
		setConnectiongNodeCanvasMode: (isConnectingNodes: boolean) => set(state => ({
			isConnectingNodes : isConnectingNodes
			})
		),
		setSelectedTask: (selectedTask : string) => set(state => ({
			selectedTask : selectedTask
		})),
		setShowDependencies: (showDependencies : boolean) => set(state => ({
			showDependencies : showDependencies
		})),
		setAllowInputToHtmlNodes: (allowInputToHtmlNodes : boolean) => set(state => ({
			allowInputToHtmlNodes : allowInputToHtmlNodes
		})),
		setFlowrunnerPaused: (isFlowrunnerPaused : boolean) => set(state => ({
			isFlowrunnerPaused : isFlowrunnerPaused
		})),
		setFlowType: (flowType : string) => set(state => ({
			flowType : flowType
		})),
		setEditorMode: (editorMode : string) => set(state => ({
			editorMode : editorMode
		})),
		setFlowsPlayground: (flowsPlayground : any[]) => set(state => ({
			flowsPlayground : flowsPlayground
		})),
		setFlowsWasm: (flowsWasm : any[]) => set(state => ({
			flowsWasm : flowsWasm
		}))
	}
};
