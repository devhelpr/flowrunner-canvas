...
let storeHandler = (set : SetState<IFlowState>) : IFlowState => {
	return {
		flow: [],
		storeFlow: (flow: any[]) => set(produce(draftState => {
				draftState.flow = FlowToCanvas.convertFlowPackageToCanvasFlow(flow);
			})
		),
		storeFlowNode : (node: any, orgNodeName: string) => set(state => {
		...
		addFlowNode : (node: any) => set(state => {
			return {
				flow: [
					...state.flow, 
					node
		...
		addConnection : (connection: any) => set(state => {
			return {
					flow: [
						...state.flow, 
						connection
		...
		deleteConnection : (node: any) => set(produce(draftState => {			
			let index = -1;
		...
		deleteNode : (node: any) => set(produce(draftState => {
			let index = -1;
			draftState.flow.map((draftNode, mapIndex) => {
				if (draftNode.name === node.name) {
					index = mapIndex;
