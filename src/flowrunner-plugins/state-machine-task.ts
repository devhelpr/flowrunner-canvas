    ...
    const promise = new Promise((resolve, reject) => {
      ...
      if (node.Expression && stateMachine.hasStateMachine) {
        ...
        if (!!node.SetInitialState) {
          ...
          stateMachine.event(result, node.payload).then((newState) => {
      ...
      if (stateMachine.hasStateMachine && node.State && !!node.SetInitialState) {
        try {
      ...
      if (stateMachine.hasStateMachine && node.Event) {
        console.log("statemachine set Event", node.Event, payload);
        stateMachine.event(node.Event, node.payload).then((newState) => {
