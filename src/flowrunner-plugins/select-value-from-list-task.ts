...
export class SelectValueFromListTask extends FlowTask {
  ...
  public execute(node: any, services: any) {
    if (node.inputProperty && node.outputProperty && node.selectvalue && 
      ...
      if (payload[node.inputProperty]) {
        ...
        const results = node.list.filter((listItem) => {
          if (listItem.selectionValue && listItem.outputValue && listItem.comparison) {
            ...
            if (listItem.comparison == "smalleroreq") {
              return inputValue <= compareValue;
            } else
            if (listItem.comparison == "eq") {
              return inputValue == compareValue;
            } else
            if (listItem.comparison == "greater") {
              return inputValue > compareValue;
            } else
            if (listItem.comparison == "greateroreq") {
              return inputValue >= compareValue;
            }
          }
