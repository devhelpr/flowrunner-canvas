...
const encodeSectionVector = (data: any[]) => {
  let flattened = flatten(data);
  return [
  ...unsignedLEB128(flattened.length),
  ...flattened
]};
...
export const generate = (
  ...
  let mainFunctionInputParameterTypes : Valtype[] = [Valtype.f32, Valtype.f32, Valtype.f32, Valtype.f32, Valtype.f32, Valtype.f32];
  if (inputVariables) {
  ...
  const typeSection = createSection(
    Section.type,
    encodeVector([mainFunctionType, sinFunctionType,...functionTypes])
  );
  ...
  const funcSection = createSection(
    Section.func,
    encodeVector([0x00 /* type index */, ...functionIndexes])
  );

  const globalsSection = createSection(
    Section.global,
    [...globals]
  );
  ...
  const exportSection = createSection(
    Section.export,
    encodeVector([
      [...encodeString("run"), ExportType.func, 0x01 /* function index .. why is this 0x01 when you import functions ??? */],
  ...
  const mainFunctionBody = encodeVector([
    localVariables.length ,
  ...
  */
  ...
  const importSection = createSection(
    Section.import,
    encodeVector([externalFunctionImport,
    ])
  );
