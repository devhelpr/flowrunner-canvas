...
export class Tween {
    ...
    constructor(config) {
        ...
        if (!layers) {
            ...
            Konva.default.Util.error('Tween constructor have `node` that is not in a layer. Please add node into layer first.');
        }
    ...
    _addAttr(key, end) {
        let node = this.node, nodeId = (node as any)._id, start, diff, tweenId, n, len, trueEnd, trueStart, endRGBA;
    ...
    _tweenFunc(i) {
        var node = this.node, attrs = Tween.attrs[(node as any)._id][this._id], key, attr, start, diff, newVal, n, len, end;
        for (key in attrs) {
            attr = attrs[key];
            start = attr.start;
            diff = attr.diff;
    ...
    destroy() {
        var nodeId = (this.node as any)._id, thisId = this._id, attrs = Tween.tweens[nodeId], key;
        this.pause();
        for (key in attrs) {
