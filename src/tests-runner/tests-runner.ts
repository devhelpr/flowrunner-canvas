...
const runTest = (testDefinition : ITestDefinition, 
		loop, 
		tests : ITestDefinition[], 
		flowRunner: FlowEventRunner) => {
	return new Promise((resolve, reject) => {
		...
		flowRunner.executeNode(testDefinition.startNode, testDefinition.payload || {}).then((payload) => {
			...
			Object.keys(testDefinition.expected).map((expectedPropertyName) => {
				...
				if (Array.isArray((payload as any)[expectedPropertyName]) && Array.isArray(testDefinition.expected[expectedPropertyName]) ) {
					if ((payload as any)[expectedPropertyName].length == testDefinition.expected[expectedPropertyName].length) {
						...
						failed = true;
						console.log("invalid length", (payload as any)[expectedPropertyName].length , testDefinition.expected[expectedPropertyName].length);
					}
				} else
				if ((payload as any)[expectedPropertyName] != testDefinition.expected[expectedPropertyName]) {
					failed = true;
					console.log("invalid values", (payload as any)[expectedPropertyName] , testDefinition.expected[expectedPropertyName]);
				};
			});
			if (failed) {
        ...
        fetch('/test?flow=' + flowId, {
          ...
          .then(response => {
			...
			if (loop < tests.length) {
				let test = tests[loop];
				const performTest = () => {
					runTest(test, loop, tests, flowRunner).then((result) => {
