    ...
    */
    app.post('/api/save-flow', (req, res) => {
      ...
      const layoutFileName =
        dirname(flowFileName) + '/layout-' + basename(flowFileName, extname(flowFileName)) + '.json';
    ...
    app.get('/api/get-preset', (req, res) => {
      ...
      const foundPresets = (presets[req.query.flowId.toString()][req.query.nodeName.toString()] || []).filter(
        (presetItem) => {
    ...
    app.post('/api/save-preset', (req, res) => {
      ...
      presets[req.query.flowId.toString()][req.query.nodeName.toString()] =
        presets[req.query.flowId.toString()][req.query.nodeName.toString()] || [];
      let found = false;
      presets[req.query.flowId.toString()][req.query.nodeName.toString()] = presets[req.query.flowId.toString()][
        req.query.nodeName.toString()
      ].map((presetItem, _index) => {
      ...
      modules.map((module) => {
        if (
          ...
          if (module.moduleType == 'object-model') {
            const moduleContent = JSON.parse(
              readFileSync(getPathToLocalFile('data/modules/' + module.fileName)).toString(),
            );
            res.send(
              JSON.stringify({
                ...module,
          ...
          } else if (module.moduleType == 'crud-model') {
    ...
    app.get('/api/modulecontent', (req, res) => {
      ...
      if (req.query.moduleId && req.query.id) {
        ...
        const modules = JSON.parse(readFileSync(getPathToLocalFile('data/modules/modules.json')).toString());
        modules.map((module) => {
          if ((module.id === req.query.moduleId || module.codeName === req.query.moduleId) && !isFound) {
            isFound = true;
            ...
            if (module.moduleType == 'crud-model') {
              if (module.datasource && module.datasource == 'flows') {
                ...
                if (itemIndex >= 0) {
                  ...
                  writeFileSync(
                    getPathToLocalFile(getPathToLocalFile('data/modules/' + module.fileName)),
        ...
        modules.map((module) => {
          ...
          if (urlProperty && urlProperty != '') {
            ...
            moduleContent.map((moduleContentItem) => {
              app.get(
                moduleContentItem[urlProperty],
                function contentRouteHandler(req, res) {
                  let primaryMenu = [];
          ...
          if (flow) {
            ...
            flow.map((node) => {
              if (node.taskType == 'RouteEndpointTask') {
                flowRoutes[node.name] = app.get(
                  node.url.indexOf('/api/') !== 0 ? `/api${node.url}` : node.url,
...
start(
  getPathToLocalFile('data/flows.json'),
  [
    {
